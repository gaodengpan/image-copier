...
**重要**：提交前运行 `make check-quality`（格式化 + 静态检查 + Lint + 竞态检测）
...
| 层 | 职责 | 禁止 | 依赖规则 |
|---|------|------|----------|
| CLI | 参数解析、调用 UseCase、输出格式化 | 业务逻辑、直接调用 Gateway | 依赖 UseCase |
| UseCase | 编排流程、业务规则校验 | 直接操作外部服务 | 依赖 Domain + Ports |
| Domain | 核心业务、实体行为、领域规则 | 依赖框架、I/O 操作 | 无外部依赖 |
| Gateway | 封装外部服务调用 | 业务逻辑 | 实现 output 接口 |

**实体行为示例**：`SyncTask.Start()`, `SyncTask.Complete()`

**关键规则**：外层可依赖内层，内层禁止依赖外层；所有具体实现通过 `AdapterFactory` 创建
...
- 提交前：`make check-quality` 必须通过

> CI 问题排查请参阅 [docs/TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md)
